...
```{r}
K = 5
gbm_model_values<-seq(0.01,0.25,0.05)
...
if( cv.svm ){
  ...
  err_cv <- array(dim=c(length(svm_gamma_values), 2))
   ...
   for(k in 1:length(svm_gamma_values)){
    cat("k=", k, "\n")
     err_cv[k,] <- cv.function(train_HOG, train_label, d = svm_gamma_values[k], K = K, cv.svm = T)
   }
}
if( cv.gbm ){
    
  err_cv <- array(dim=c(length(gbm_model_values), 2))
  
   for(k in 1:length(gbm_gamma_values)){
    cat("k=", k, "\n")
     k = 1
     err_cv[k,] <- cv.function(train_data, train_label, d = gbm_model_values[k], K = K, cv.svm = T)
   }
}
...
if(run.tf){
  tm_train <- system.time(MODEL <- train(train_data, train_label, run.tf=T))
}
if(run.XGB){
...
if(run.test){
  tm_test <- system.time(pred_test <- test(MODEL, test_data, test.GBM = run.GBM, 
                 test.RF = run.RF, test.TF = run.tf, test.Ada = run.Ada, test.XGB = run.XGB))
